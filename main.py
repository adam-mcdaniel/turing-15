...
def g_or_l(var):
	...
	if "." in var:
		var = str(var[:var.index(".")])

	if "[" in var:
		print(str(var))
		var =  var.replace(var[var.index('['):var[::-1].index(']')-1]+']', "")
		# print("changed to: " + str(var)+":"+str(scope))

	if var in class_vars:
		return "this."+store_var

	print("global: "+store_var)
	return store_var
...
def g_or_l_args(list_vars):
	...
	for var in list_vars:
		...
		if var in class_vars:
			list_vars_new.append("this."+store_var)
		else:
...
dif_imported_require =  '(' allw '?require' w string:a w ')' -> ''
...
setmethod = '(' allw 'set' allw (allw set|set_list|set_function_call|comment|force_args)*:a allw ')' allw -> '\\n'.join(a)+'\\n'
...
imported = ( allw comment
			...
			| allw dif_imported_require:a endl allw  -> a
			| allw functioncall:a endl allw  -> a + " \\n"
			)
